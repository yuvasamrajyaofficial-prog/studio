...
export function EnhancedStarfield() {
  ...
  useEffect(() => {
    ...
    const newStars: Star[] = Array.from({ length: starCount }, (_, i) => ({
      ...
      x: Math.random() * 100,
      y: Math.random() * 100,
      size: Math.random() * (mobile ? 1.5 : 2) + 0.5,
    ...
    <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
      ...
      {!isMobile && (
        <div className="absolute inset-0 opacity-20">
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-purple-900/30 rounded-full blur-[120px] animate-pulse" style={{ animationDuration: "6s" }} />
          <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-amber-900/20 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: "3s", animationDuration: "6s" }} />
        </div>
      )}

      {/* Stars - with GPU acceleration */}
      <div className="absolute inset-0">
        {stars.map((star) => (
          <motion.div
            ...
            animate={{
              opacity: [star.opacity, Math.min(1, star.opacity + 0.3), star.opacity],
              scale: [1, 1.2, 1],
            }}
            transition={{
